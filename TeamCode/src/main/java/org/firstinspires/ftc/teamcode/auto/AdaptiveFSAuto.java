        ...
        if (tunnelCycles > 0) {
            ...
            for (int tunnelIndex = 0; tunnelIndex < tunnelCycles; tunnelIndex++) {
                ...
                if (tunnelIndex % 2 == 1) {
                    builder = builder
                            .stopAndAdd((() -> bot.sensorIntake(true)))
                            .splineTo(new Vector2d(Pos.blueSecretTunnel.position.x - secretTunnelOffset, Pos.blueSecretTunnel.position.y), Math.toRadians(90), drive.defaultVelConstraint, new ProfileAccelConstraint(-70, 70))
