...
//
    ...
    private Action buildFarAuto(MecanumDrive drive, boolean isBlue, AutoConfig cfg) {
        ...
        if (cfg.runHp) {
            builder = builder
                    .strafeToConstantHeading(Pos.midShoot)
                    .stopAndAdd(new InstantAction(() -> bot.intake.intake()))
            ...
            if (cfg.runClose) {
                ...
                builder = builder
                        .afterTime(0.1, bot.enableShooter())
                        .strafeToLinearHeading(Pos.farShoot, Math.toRadians(90))
                        .stopAndAdd(bot.shootThreeAutoFar())
                        .stopAndAdd(bot.shootThreeAutoClose());
            }
        ...
        if (cfg.runFar) {
            if (isBlue) {
                builder = builder
                        .stopAndAdd(new InstantAction(() -> bot.intake.intake()))
                        .splineTo(Pos.blueFarIntakeFar.position, Math.toRadians(90))
                        .strafeToConstantHeading(new Vector2d(Pos.blueFarIntakeFar.position.x,Pos.blueFarIntakeFar.position.y + 22))
                        .strafeToLinearHeading(Pos.farShoot, Math.toRadians(90))
                        .stopAndAdd(bot.shootThreeAutoFar())
                        .stopAndAdd(bot.shootThreeAutoClose())
                        .stopAndAdd(new InstantAction(() -> bot.intake.intake()))
                ...
                builder = builder
                        ...
                        .strafeToConstantHeading(new Vector2d(Pos.redFarIntakeFar.position.x,Pos.redFarIntakeFar.position.y + 22))
//                        .setReversed(true)
                        .strafeToLinearHeading(Pos.farShoot, Math.toRadians(90))
                        .stopAndAdd(bot.shootThreeAutoFar())
                        .stopAndAdd(bot.shootThreeAutoClose())
        ...
        if (cfg.runCycles) {
            builder = builder
                    ...
                    .splineToSplineHeading(Pos.blueHpIntakeCycles, Math.toRadians(180))
                    .strafeToConstantHeading(Pos.farShoot)
                    .stopAndAdd(bot.shootThreeAutoFar())
